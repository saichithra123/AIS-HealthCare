...
const ControlType = () => {
            ...
            renderCell: (params) => {
                return (
                    <Button
                        ...
                        onClick={() => {
                            navigate(`/configurations/control-type/add`, {
                                state: { controlTypeId: params.row?.controlTypeId },
                            });
                        }}
                    >
                        Edit
    ...
    const getControlTypes = async () => {
        const response = await axios.get(`${import.meta.env.VITE_BASE_URL}/getAllControlType?page=${page - 1}&size=${pageSize}`);
        setControlTypes(response?.data?.content);
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        >
                            <Button
                                onClick={() => navigate("/configurations/control-type/add")}
                                variant="contained"
                                startIcon={<AddIcon />}
                                sx={{
                                    backgroundColor: `${theme.palette.colors[11]}`,
                ...
                >
                    <DataGrid
                        columns={columns}
                        rows={controlTypes}
                        getRowId={(row) => row.controlTypeId}
                        loading={loading}
                        checkboxSelection
